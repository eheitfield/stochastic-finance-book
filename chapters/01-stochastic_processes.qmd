...
## Stochastic processes: random functions that evolve over time

Most of the time in applied work you will not be working directly with probability spaces, but you will absolutely be working with stochastic processes. These are the building blocks we use to construct all continuous-time finance models.
...
## Example: the Poisson process {#sec-poisson_process}

A Poisson process, is an example of a stochastic process which describes the occurrence of random events through time. We'll put it to use later in this book to model jumps in asset prices.
...
Consider the Poisson process, $\{N_t\}$, we introduced in @sec-poisson_process. Suppose we wish to predict the number of events that will have occurred by a later time $T>t$. At time $t$, we already know that $N_t$ events have occurred. The only uncertainty comes from the future arrivals between $t$ and $T$. Because the increments of a Poisson process are independent,
