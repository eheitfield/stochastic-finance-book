# Dicsounting Future Cash Flows

The Fundamental Theorem of Asset Pricing places discounted future payoffs at the center of asset valuation. In the previous chapter, and in many of the models to come, we assume that a risk-free asset grows a constant, deterministic interest rate $r$.  This assumption simplifies the math of discounting future cash flows, enabling to focus on other important model features.  But it not particularly realistic over anything but a very short time horizon.  @fig-effr shows the effective federal funds rate, a overnight interest rate managed by the Federal Reserve, from 1970 to 2026.  While it was low and stable for nearly a decade following the global financial crisis in 2008, it has fluctuated over a considerable range at other times.  In this chapter we show how future cash flows can be discounted when the instantaneous short rate is a stochastic process $\{r_t\}$ that evolves over time.
...
Now suppose the instantaneous rate of growth of money deposited in a risk-free account -- the **short rate** -- evolve over time according to the deterministic function $r_t$.  If our account contains $V_t$ at time $t$ then over a short interval we will receive $r_t\,dt$ in interest.  If we invest $V_0$ dollars in the risk-free account and immediately reinvest all interest received so that it compounds, then the value in our account at time $T$ will be approximately equal to
...
The exact formula for $V_T$ derives from the limit
...
But what if we don't know the short-rate path in advance?  Then, of course, $\{r_t\}$ is a random process, and so is the discount factor $\{D_{t,T}\}$.  The pricing formula for an uncertain payout $X_T$ becomes
...
One implication of the result above is that the discounted value of a future payout $X_T$ is a random variable, even if $X_T$ is known in advance.  Even if one invests in very safe assets, such as U.S. Treasury securities, with certain future case flows, one still bares economic risk because of uncertainty about the future path of interest rates.

## Real bonds and the term structure of interest rates {#sec-yield_curve}
...
$$
When you read about the *yield curve* in the financial press, you are reading about the relationship between time to maturity $(T-t)$ and the prices of risk-free bonds.^[In practice, published yields are usually computed from the traded prices coupon-paying Treasury securities rather than directly from zero-coupon bond prices.]  As shown in @fig-yield_curve_ex, the yield curve may be upward- or downward-sloped or non-monotonic, depending on the $Q$-measure expected short-rate path at the time it is computed.

![U.S. Treasury yield curves for selected dates.](../figures/yield_curve_ex.svg){#fig-yield_curve_ex}

Consider two alternative one-dollar investments made at time $t$. In the first case, we invest one dollar in a zero coupon bond maturing at date $T$ with price ($P_{t,T}$) and then immediately reinvest the proceeds in a new bond that matures at $T+\Delta T$ with a time-$T$ price of $F(T,T+\Delta T)$.  In the second case, we invest one dollar directly in a bond that matures at $T+\Delta T$ whose price is $P(t,T+\Delta T)$.  Because both investments have a value of one dollar today, and both investments are risk free, they must have the same value at $T+\Delta T$.  Thus

$$
P_{t,T}\,F_{T,T+\Delta T} = P_{t,T+\Delta T}.
$$
The formula for the zero-coupon bond yield implies
...
$$
$f_{t,T}, it is the market rate at date $t$ for guaranteed short-term funding at date $T$. In principle, one could lock in this rate using a portfolio of risk free-bonds, though in practice this is more commonly done using interest rate futures.

Notice that
$$
y_{t,T} = \frac{1}{T-t}\int_t^T f_{t,s} ds
$$
so we can think of the zero yield from as an average of instantaneous forward rates.
...
## Application: Two views of duration and convexity
...
The first task in measuring a bond's sensitivity to changes in interest rates is to answer the question, "which interest rates?"  As we have just seen, a time-varying short rate path implies an entire term structure of interest rates, and it is entirely possible that rates at different points on the term structure may move differently. In practice, duration and convexity are most commonly defined with respect to a parallel shift in the entire risk-free yield curve. If, at time $t$, the yield curve is perturbed by an amount $\Delta y$, then each discount factor $D_{t,T}$ is shifted to $D_{t,T}d^{\Delta y(T-t)}$, so $\frac{\partial}{\partial y} D_{t,T} = -D_{t,T}(T-t)$.
...
Convexity measures the curvature of a bond’s price-yield relationship and captures the second-order effect of small change in yield on the bond’s price.
...
@fig-d_and_c shows an example of how a bond's price might change for a given parallel shift in the yield curve.  A second-order approximation of this change will can be expected to perform better for small values of $\Delta y$ than a simple linear approximation that reflects only the effect of duration, but there is no guarantee that this will be true for larger yield curve shifts.
...
Notice that $\int_t^T w(s) ds = 1$ so we can think of $w(s)$ as a weighting function.  If we impose the assumption that $c_t \geq 0$ for all $t$, as is true for most bonds, then $w(s)ds$ is the share of the bond's current value contributed by cash flows that arrive in a short time interval near $s$. In fact, $w(s)$ a valid probability density function, and bond duration and convexity can be interpreted as moments. 
...
$$
These are not moments of the net cash flow, but instead are differences of the moments of the positive and negative cash flow components weighted by each component's contribution to the present value of the net position.

## Application: The economic difference between futures and forwards
...
A forward contract is similar to a European call option except that the there is no optionality.  At date $t$, the counterparty who is long a forward contract agrees to pay a fixed price $K_t$ at date $T$ in exchange for a specified quantity of an asset whose future value is $S_T$. In the absence of arbitrage, $K_t$ must be set so that the value of the forward contract is zero at date $t$.  Otherwise either the buyer or the seller would earn positive risk-neutral returns. Thus, using our standard $Q$-measure valuation formula,
...
$$
An implication of this is that $\{F_t\}$ must have zero drift.  Otherwise someone could enter into the contract with a long or short position at no cost and instantly receive a positive $Q$-measure expected payment, which would violate the no arbitrage condition. Thus
$$
E_t^Q[dF_t]=0
$$ 
