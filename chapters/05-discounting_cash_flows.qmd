...
## The economic difference between futures and forwards
...
The FTAP implies that if $S_t$ is an asset that generates no intermediate benefit flows (such as dividends) or cost flows (such as storage costs) then its discounted value is a martingale under the $Q$ measure. Substitution the martingale condition $S_t = \E_^Q[D_{t,T}S_T]$ into the expression for $K_t$ gives us another way to interpret the forward price:

$$
K_t = \frac{S_t}{P_{t,T}}.
$$

Under no arbitrage, the forward price must be equal to the opportunity cost (in terms of foregone risk-free interest income) of buying the asset at $t$ and holding it until $T$.
