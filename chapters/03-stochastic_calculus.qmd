...
[^03-stochastic_calculus-2]: The observant reader will notice that throughout this chapter the symbol “=” is used in two different ways. When writing equations involving stochastic differentials, expressions such as $dW_t^2 = dt$ should not be interpreted as ordinary algebraic identities. Rather, they are convenient shorthand for the limiting behavior of Wiener increments described by quadratic variation. Once these rules have been established, they may be manipulated algebraically when working with stochastic differential equations.
...
Applyng all of these rules to our earlier example, we have
...
weights values of $f(t)$ by increments of an integrator function $g(t)$. Somewhat less common than the Riemann integrals, Riemann-Stieltjes integrals are widely used in probability and statistics to compute expectations of continuous random variables.
